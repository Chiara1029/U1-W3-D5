...
function deleteOne(str, bool) {
  if (bool === true) {
    const splitted = str.split("");
    splitted.shift();
    const backtoString = splitted.join("");
...
function rollTheDices(n) {
  ...
  const totalDicesRolls = {};
  totalDicesRolls.sum = sum;
  totalDicesRolls.value = values;

  return totalDicesRolls;
}
...
//GLI ESERCIZI DAL 12 IN POI SONO STATI SPOSTATI SOTTO L'OGGETTO
...
function addBackground() {
  const linksTo = document.querySelectorAll("a");
  linksTo.style.backgroundColor = "red";
}
...
function addToList() {
  const thisIsMyList = document.getElementById("myList");
  const thisIsMyContent = document.createElement("li");
  thisIsMyContent.innerText = "New Li";

  thisIsMyList.appendChild(thisIsMyContent);
}
